...
| [OpenAPI 3.2](https://spec.openapis.org/oas/v3.2.0.html) | HTTP operations, parameters, request bodies, responses, security, and versioned interfaces have a portable structural representation | OpenAPI documents project to authoring evidence with explicit read/write effects; projection does not resolve remote references or call the API |
| [CloudEvents 1.0](https://github.com/cloudevents/spec) | Event identity, source, type, schema, and content type can be represented independently from a transport | Supplied envelopes project to event-type operations; delivery, signature, producer authority, and transport semantics remain adapter responsibilities |
| [BPMN 2.0.2](https://www.omg.org/spec/BPMN) | Human, service, event, and gateway workflows have a standardized interchange structure and richer engine semantics | The reference adapter projects supported flow nodes and dependencies only; token, timer, transaction, and compensation semantics require a BPMN runtime |
| [Kubernetes Jobs](https://kubernetes.io/docs/concepts/workloads/controllers/job/) | Batch execution, retries, parallelism, and completion are scheduler/runtime concerns | A frozen plan exports an exact portable task manifest; an authorized adapter must create Jobs and enforce runtime, secret, retry, and isolation policy |
